...
def train_one_epoch(model, data_loader, optimizer, device, epoch,
    ...
    for data_iter_step, (data_dcit) in enumerate(data_loader):
        ...
        if config.do_sup_contrast:
            ...
            positive_fmri_feats = [all_fmri_features[sindex[1:num_sel_pos_contrast + 1]] for sindex in
                                   positive_sample_indexes]
            negative_fmri_feats = [all_fmri_features[sindex[:num_sel_neg_contrast]] for sindex in
            ...
            positive_fmri_feats = torch.FloatTensor(
                np.pad(positive_fmri_feats.squeeze(1), ((0, 0), (0, num_voxels - positive_fmri_feats.shape[-1])),
                       'wrap'))
            negative_fmri_feats = torch.FloatTensor(
                np.pad(negative_fmri_feats, ((0, 0), (0, 0), (0, num_voxels - negative_fmri_feats.shape[-1])), 'wrap'))
        ...
        with torch.cuda.amp.autocast(enabled=True):
            loss, pred, _, mask_loss, contrast_loss, distill_loss = model(samples, mask_ratio=config.mask_ratio,
                                                                          pos_sample=positive_fmri_feats,
                                                                          neg_sample=negative_fmri_feats,
                                                                          distill_pos_sample=distill_pos_sample,
                                                                          dropout=True)
        # loss.backward()
...
def train_one_epoch_contrast(model, data_loader, optimizer, device, epoch, loss_scaler,
                            ...
                            num_sel_pos_contrast=1, num_sel_neg_contrast=4, time_len=512, do_test=False,
    ...
    for data_iter_step, (data_dcit) in enumerate(data_loader):
        ...
        if config.do_sup_contrast:
            ...
            positive_fmri_feats= torch.FloatTensor(np.pad(positive_fmri_feats.squeeze(1), ((0,0), (0, time_len - positive_fmri_feats.shape[-1])), 'wrap'))
            negative_fmri_feats= torch.FloatTensor(np.pad(negative_fmri_feats, ((0,0), (0,0), (0, time_len - negative_fmri_feats.shape[-1])), 'wrap'))
        ...
        with torch.cuda.amp.autocast(enabled=True):
            loss, pred, _, contrast_loss = model(samples, mask_ratio=config.mask_ratio)
