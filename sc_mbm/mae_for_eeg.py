...
class MAEforEEGContrast(nn.Module):
    ...
    def __init__(self, time_len=512, patch_size=4, embed_dim=1024, in_chans=128,
                 ...
                 do_distill_contrast=False, distill_contrast_loss_weight=0.1,
                 do_mask_loss=False, mask_loss_weight=0.5,
                 do_sup_contrast=False, num_sel_neg_contrast=5,
                   fmri_channels =263, channels = 256,
                  negative_mode='unpaired', decoder_freeze=False,
                 sep_loss=False):
        super().__init__()
        ...
        self.do_cross_contrast = do_cross_contrast # 2
        self.self_contrast_loss_weight = self_contrast_loss_weight # 3
        self.cross_contrast_loss_weight = cross_contrast_loss_weight # 4
    ...
    def getLoss(self, query, pos, neg, temperature=0.1, reduction='mean'):
        positive_logit = torch.sum(query * pos, dim=1, keepdim=True)

        query = query.unsqueeze(1)

        negative_logits = query @ transpose(neg)
    ...
    def get_info_nce(self, query, pos, neg=None, temperature=0.1, reduction='mean'):
        ...
        for i in range(len(query_list)):
            loss_tmp = self.getLoss(query_list[i], pos_list[i], neg_list[i])
    ...
    def info_nce(self, query, positive_key, negative_keys=None, temperature=0.1, reduction='mean', negative_mode='unpaired'):
        ...
        if negative_keys is not None:
            if negative_mode == 'unpaired' and negative_keys.dim() != 3:
                raise ValueError("<negative_keys> must have 3 dimensions if <negative_mode> == 'unpaired'.")
            if negative_mode == 'paired' and negative_keys.dim() != 4:
        ...
        for i in range (len(query)):
            ...
            query_tmp = normalize(query[i])[0]
            positive_key_tmp = normalize(positive_key[i])[0]
            query_list.append(query_tmp)
            positive_key_list.append(positive_key_tmp)

            if negative_keys is not None:
                negative_keys_tmp_list = []
                for j in range(len(negative_keys[i])):
                    negative_keys_tmp = normalize(negative_keys[i][j])[0]

                    negative_keys_tmp_list.append(negative_keys_tmp)
                negative_keys_tmp2 = torch.stack(negative_keys_tmp_list, dim=0)
                negative_keys_list.append(negative_keys_tmp2)

        query = torch.stack(query_list, dim=0)
        positive_key = torch.stack(positive_key_list, dim=0)


        if negative_keys is not None:
            negative_keys = torch.stack(negative_keys_list, dim=0)
        ...
        if negative_keys is not None:
            ...
            if negative_mode == 'unpaired':
                ...
                info_nce = self.get_info_nce(query, positive_key, negative_keys, temperature)

            #     query = query.unsqueeze(1)
            #     negative_logits = query @ transpose(negative_keys)
            #     negative_logits = negative_logits.squeeze(1)
            #
            # # First index in last dimension are the positive samplests = torch.cat([positive_logit, negative_logits], dim=1)
            # logits = torch.cat([positive_logit, negative_logits], dim=1)
            # labels = torch.zeros(len(logits), dtype=torch.long, device=query.device)
        else:
            ...
            # print('nxn_cos_sim')
            print('negative keys is None')
            logits = self.nxn_cos_sim_3d(query, positive_key)
            ...
            labels = torch.tensor(np.arange(1, rows * cols + 1).reshape(rows, cols), device=query.device)
    ...
    def forward_contrast_loss(self, pred, pos_sample, neg_sample=None, unpatch=True, temperature=0.1):
        ...
        if self.negative_mode == 'paired':
            ...
            loss = self.info_nce(pred, pos_sample, neg_sample, temperature=temperature, reduction='mean',
    ...
    def nxn_cos_sim_3d(self, A, B):
        ...
        for i in range(A.shape[0]):
            tmp = self.nxn_cos_sim(A[i], B[i])
            result.append(tmp)

        cos_sim = torch.stack(result, dim=0)
        return cos_sim
    ...
    def forward_distill_loss(self, fmri_map, pos_dis_sample, neg_dis_sample=None, temperature=0.5):
        ...
        pos_distill_loss = self.distill_loss(F.log_softmax(fmri_map / temperature, dim=1),
                                             F.softmax(pos_dis_sample / temperature, dim=1))
            ...
            if pred.shape[0] > 1:
                if self.negative_mode == 'paired':
                    for ii in range(pred.shape[0]):
                        ...
                        pred_neg2.append(self.unpatchify(torch.stack(pred_neg_ii)).squeeze(1))

                    pred_neg2 = torch.stack(pred_neg2)

            cross_contrast_loss = self.cross_contrast_loss_weight * self.forward_contrast_loss(pred, pred_pos, pred_neg2)
            if self.sep_loss and self.do_self_contrast:
                # print('cross pred img shape', pred_pos.shape, imgs.squeeze(1).shape)
                cross_contrast_loss += self.self_contrast_loss_weight * self.forward_contrast_loss(pred_pos,
                                                                                                   imgs.squeeze(1),
                                                                                                   unpatch=False)
...
class MAEforEEGCross(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone
    """
    ...
    def __init__(self, time_len=512, patch_size=4, embed_dim=1024, in_chans=128,
        ...
        if self.do_cross_attention:
            self.cross_blocks = nn.ModuleList(
                [ViTMAELayer(cross_encoder_config, True) for _ in range(cross_encoder_config.num_cross_encoder_layers)])
    ...
    def initialize_weights(self):
        ...
        decoder_pos_embed = ut.get_1d_sincos_pos_embed(self.decoder_pos_embed.shape[-1], self.patch_embed.num_patches,
                                                       cls_token=True)
        self.decoder_pos_embed.data.copy_(torch.from_numpy(decoder_pos_embed).float().unsqueeze(0))

        if self.use_nature_img_loss:
            nature_img_decoder_pos_embed = ut.get_1d_sincos_pos_embed(self.nature_img_decoder_pos_embed.shape[-1],
                                                                      self.patch_embed.num_patches, cls_token=True)
            self.nature_img_decoder_pos_embed.data.copy_(
                torch.from_numpy(nature_img_decoder_pos_embed).float().unsqueeze(0))
            torch.nn.init.normal_(self.nature_img_mask_token, std=.02)
