...
def get_eeg_latent(args):
    ...
    condition, re_latent = generative_model.get_latent_name(eeg_latents_dataset_train, eeg_latents_dataset_test, config.num_samples, image_name=args.image_name)
...
def main():
    ...
    with torch.no_grad():
        ...
        factor = max(args.particle_num_vsd,args.particle_num_phi)
        eeg_embeddings = get_eeg_latent(args)
        text_embeddings = eeg_embeddings.expand(factor, -1, -1)
    ...
    print(f'text_embeddings_vsd {text_embeddings_vsd.shape}')
    print(f'text_embeddings_phi {text_embeddings_phi.shape}')
    ...
    ### sds text to image generation
        ...
        for step, chosen_t in enumerate(pbar):
            ...
            print(f'7987latents{latents.shape}')
            ...
            print(f'latents_vsd{latents_vsd.shape}')
