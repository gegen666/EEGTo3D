...
def double_form_dataloader(updir, image_size, batch_size, mode, 
                           read_channel='color', data_aug=True, shuffle=True, drop_last=True):
    dataset = Double_dataset(updir, image_size, mode, read_channel, data_aug)
...
class ImageNetDataset(Dataset):
    def __init__(self, root_dir, image_name, transform=None):
        self.root_dir = root_dir
        self.image_files = [os.path.join(root_dir, f) for f in os.listdir(root_dir) if
                            f.endswith(('.png', '.jpg', '.jpeg', '.JPEG', '.PNG', '.JPG'))]

        # self.image_files = [os.path.join(root_dir, f) for f in os.listdir(root_dir) if
        #                     f.startswith(image_name)]
...
def image_dataloader(up_dir, train_bc, eval_bc, to_shuffle):
    ...
    train_dataloader = DataLoader(dataset, batch_size=train_bc, shuffle=to_shuffle)
    test_dataloader = DataLoader(test_dataset, batch_size=eval_bc, shuffle=to_shuffle)
...
def get_eeg_latent_imageDataloader(config, train_bc, eeg_dataset_train, eeg_dataset_test, image_name):
    ...
    return eeg_latent, train_dataloader, train_dataloader  # , test_dataloader