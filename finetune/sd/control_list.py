...
def main(args):
    ...
    controlnet_slo = ControlNetModel.from_pretrained(cf['control_resume'])
    controlnet_first = ControlNetModel.from_pretrained(cf['control_resume'])
    
    control_list = MultiControlNetModel([controlnet_slo, controlnet_first])
    ...
    optimizer = torch.optim.AdamW(control_list.parameters(), lr=cf["lr"])
    ...
    control_list, optimizer, train_dataloader, lr_scheduler, val_dataloader = accelerator.prepare(
            control_list, optimizer, train_dataloader, lr_scheduler, val_dataloader
        )
    global_step = 0
    ...
    for epoch in range(cf['num_epochs']):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(control_list):
                ...
                down_block_res_samples, mid_block_res_sample = control_list(
                    noisy_latents, 
                    timesteps, 
                    encoder_hidden_states=text_embeddings,
                    controlnet_cond=[slo, first_ffa],
                ...
                accelerator.clip_grad_norm_(control_list.parameters(), 1.0)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
        ...
        if accelerator.is_main_process:
            ...
            pipeline = StableDiffusionControlNetPipeline(autoencoderkl_tar, 
                                                         ...
                                                         tokenizer, 
                                                         model,
                                                         accelerator.unwrap_model(control_list), 
                                                         noise_scheduler, 
                                                         None, 
                                                         None,
                                                         False
            ...
            if (epoch + 1) % val_interval == 0 or epoch == cf['num_epochs'] - 1:
                ...
                images = pipeline(prompt, [slo, first_ffa], 
                                  num_inference_steps=100, guidance_scale=1.0, 
                                  output_type='pt').images 
                images = torch.cat([slo, first_ffa, (images-0.5)/0.5, targets], dim=-1)
            ...
            if (epoch + 1) % save_interval == 0 or epoch == cf['num_epochs'] - 1:
                if cf['only_save_part']:
                    accelerator.unwrap_model(control_list).save_pretrained(j(cf['project_dir'], 'controlnet')) 
                    noise_scheduler.save_pretrained(j(cf['project_dir'], 'scheduler')) 
                else:
                    pipeline.save_pretrained(j(cf['project_dir'], 'controlnet'))   
    
    
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--config_path', type=str, default='config/control_list_config.yaml')
