...
class eLDM:
    ...
    def __init__(self, metafile, num_voxels, device=torch.device('cpu'),
        ...
        # print(f"cond_stage_model444 {model.cond_stage_model}") eeg encoder
    ...
    def get_latent(self, fmri_embedding, num_samples, limit=None):
        model = self.model.to(self.device)
        print(len(fmri_embedding))
        c_list=[]
        re_latent_list = []
        with model.ema_scope():
            model.eval()
            for count, item in enumerate(fmri_embedding):
                ...
                c, re_latent = model.get_learned_conditioning(repeat(latent, 'h w -> c h w', c=num_samples).to(self.device))
                c_list.append(c)
                re_latent_list.append(re_latent)

        return c_list, re_latent_list
